...
アクティビティを監査するために各ページを手動で進め [!DNL Target] のを避けるには、Adobe Auditor を使用して、実装の現在の技術的な状況を理解するようにします。 Adobe Auditor は ObservePoint を利用しており、手動レベルで実行するように設定して、サイトの実装に関する大まかな問題を特定できます。
