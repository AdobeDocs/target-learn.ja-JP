...
`at.js` 2.0は、Adobe Targetのシングルページアプリケーション（SPA）のサポートを強化し、他のExperience Cloud ソリューションと統合します。 このビデオと図では、すべてがどのようにまとまっているかを説明します。
...
1. 呼び出しはExperience Cloud ID （ECID）を返します。 ユーザーが認証された場合、別の呼び出しは顧客IDを同期します。
...
1. プロファイルスクリプトが実行され、[!UICONTROL &#x200B; プロファイルストア &#x200B;]に入力されます。 ストアは、[!UICONTROL Audience Library]から適格なオーディエンスをリクエストします（例：[!DNL Analytics]やAudience Managerから共有されたオーディエンス）。 [!UICONTROL 顧客属性]は、バッチプロセスで[!UICONTROL &#x200B; プロファイルストア &#x200B;]に送信されます。
...
1. ページから[!UICONTROL &#x200B; データ収集] サーバーに[!DNL Analytics]個のデータが送信されました
...
1. アクティビティおよび増分指標で訪問者をカウントするために、[!DNL Target] [!UICONTROL &#x200B; プロファイルストア &#x200B;]に通知リクエストが送信されます
1. [!DNL Analytics] データがSPAから[!UICONTROL &#x200B; データ収集] サーバーに送信されます

1. [!DNL Target] データが[!DNL Target] バックエンドから[!UICONTROL &#x200B; データ収集] サーバーに送信されます。 [!DNL Target] データはSDIDを介して[!DNL Analytics] データと照合され、[!DNL Analytics] レポートストレージに処理されます。 [!DNL Analytics]個のデータは、A4T レポートを使用して[!DNL Analytics]と[!DNL Target]の両方で表示できます。
