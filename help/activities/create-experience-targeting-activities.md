...
description: XT アクティビティの利点と、作成および使用方法について説明します。 エクスペリエンスのターゲット設定（XT）アクティビティを使用すると、マーケターは特定のコンテンツを特定のオーディエンスにターゲット設定できます。
role: User
level: Beginner
...
# エクスペリエンスのターゲティングアクティビティの作成

エクスペリエンスのターゲット設定は、パーソナライゼーションの第一歩であり、多くの場合、A/B テストから始まります。 マーケティング担当者は A/B テストを実施し、レポート用オーディエンスを使用することで、エクスペリエンスが異なれば、それに共感するオーディエンスも異なることを発見します。 エクスペリエンスのターゲット設定（XT）により、マーケターはその発見を簡単に活用し、それらの異なるユーザーセグメントに対するコンテンツの長期的なターゲティングに切り替えることができます。

## 学習内容

このビデオでは、次の方法を学びます。

* Adobe Target Visual Experience Composer （VEC）を使用してExperience Targeting （XT）アクティビティを作成する
